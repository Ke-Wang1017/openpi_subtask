...
class LeRobotInferenceObserver:
    ...
    def load_lerobot_episode(self, episode_path: str, frame_idx: int = 0) -> Dict[str, Any]:
        ...
        try:
            # 尝试加载 HDF5 文件
            if episode_path.endswith('.hdf5') or episode_path.endswith('.h5'):
    ...
    def _create_fake_lerobot_data(self) -> Dict[str, Any]:
        """创建模拟的 LeRobot 数据"""
        return {
    ...
    def prepare_state_from_lerobot(self, episode_data: Dict[str, Any], frame_idx: int = 0) -> Optional[np.ndarray]:
        """从 LeRobot 数据准备状态"""
        if 'state' in episode_data:
            state_data = episode_data['state']
            if len(state_data.shape) == 2:  # (T, state_dim)
                return state_data[frame_idx]
            elif len(state_data.shape) == 1:  # (state_dim,)
                return state_data
            else:
    ...
    async def observe_single_inference(self, 
                                     episode_data: Dict[str, Any],
                                     frame_idx: int = 0,
                                     high_level_prompt: Optional[str] = None,
                                     low_level_prompt: Optional[str] = None,
    ...
    async def observe_continuous_inference(self,
                                         episode_data: Dict[str, Any],
                                         start_frame: int = 0,
                                         max_frames: int = 10,
                                         ...
                                         high_level_prompt: Optional[str] = None,
                                         low_level_prompt: Optional[str] = None,
                                         subtask_refresh_interval: Optional[float] = None) -> List[Dict[str, Any]]:
        """持续观察推理过程"""
        await self.initialize()
    ...
    async def _save_observation(self, observation: Dict[str, Any]):
        ...
        serializable_data = {
            ...
            "result": {
                "actions": observation['result']['actions'].tolist() if observation['result']['actions'] is not None else None,
                "subtask": observation['result']['subtask'],
                "subtask_tokens": observation['result']['subtask_tokens'].tolist() if observation['result']['subtask_tokens'] is not None else None,
                "state": observation['result']['state'].tolist() if observation['result']['state'] is not None else None,
...
async def main():
    ...
    observer = LeRobotInferenceObserver(
        ...
        gpu_id=1,
        output_dir="./lerobot_inference_outputs"
    )
    
    # 添加观察回调
    async def on_observation(data):
        logger.info(f"📊 收到观察数据:")
    ...
    # 创建模拟的 LeRobot 数据
    fake_episode = observer._create_fake_lerobot_data()
    ...
    try:
        ...
        single_obs = await observer.observe_single_inference(
