...
class AsyncPi05Inference:
    ...
    async def initialize(self):
        ...
        # JIT 编译关键函数
        self.jit_sample_low_level_task = nnx_utils.module_jit(
            self.model.sample_low_level_task, static_argnums=(3,)
        )
    ...
    def prepare_observation(self, 
                          ...
                          state: Optional[np.ndarray] = None) -> Observation:
        """准备观察数据"""
        ...
        if state is None:
            ...
            state = jnp.array(state)[np.newaxis, :]
        ...
        observation = _model.preprocess_observation(None, new_observation, train=False, 
                                                  image_keys=list(observation.images.keys()))
        observation = jax.tree.map(jax.device_put, observation)
        
        return observation
    
    async def generate_subtask(self, 
                              observation: Observation, 
                              rng: jax.Array,
                              max_decoding_steps: int = 200,
                              temperature: float = 0.1) -> Tuple[jnp.ndarray, str]:
        """生成子任务"""
        start_time = time.time()
        
        # 生成子任务 tokens
        predicted_token, kv_cache, mask, ar_mask = self.jit_sample_low_level_task(
    ...
    async def infer(self, 
                   ...
                   subtask_refresh_interval: Optional[float] = None) -> Dict[str, Any]:
        """
        异步推理函数，支持子任务生成和定期刷新
        ...
        if subtask_refresh_interval is not None and subtask_refresh_interval > 0:
            ...
            results["subtask_refresh_task"] = asyncio.create_task(
                self._periodic_subtask_refresh(
                    ...
                    subtask_refresh_interval, max_decoding_steps, temperature
                )
            )
    ...
    async def _periodic_subtask_refresh(self, 
                                      images: Dict[str, np.ndarray],
                                      high_level_prompt: str,
                                      low_level_prompt: str,
        ...
        while True:
            try:
                ...
                await self._on_subtask_refresh(
                    subtask_text, subtask_tokens, refresh_count, 
                    subtask_text, None  # 不传递动作
                )
    ...
    async def _on_subtask_refresh(self, subtask_text: str, subtask_tokens: jnp.ndarray, refresh_count: int, 
                                 updated_low_prompt: str, updated_actions: jnp.ndarray):
        """子任务刷新回调函数，可以被子类重写"""
        # 默认实现：只记录日志
    ...
    async def start_continuous_action_generation(self, 
                                               images: Dict[str, np.ndarray],
                                               high_level_prompt: str,
                                               low_level_prompt: str,
                                               state: Optional[np.ndarray] = None,
